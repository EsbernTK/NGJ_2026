...
func _get_description():
	return "Classic Simplex-Noise-3D function (by Curly-Brace)"
...
func _get_code(input_vars, output_vars, mode, type):
	return output_vars[0] + " = snoise(vec3((%s.xy + %s.xy) * %s, %s)) * 0.5 + 0.5;" % [input_vars[0], input_vars[1], input_vars[2], input_vars[3]]
	#return output_vars[0] + " = 0.0f;"
